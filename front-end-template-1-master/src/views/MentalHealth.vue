  ...
  methods: {
    ...
    async loadDashboardData() {
      ...
      try {
        ...
        const res = await $http.get('/mental/dashboard/overview')
        const data = res.data || {}
    ...
    async loadEmotionDistribution() {
      ...
      try {
        // 请求情绪分布接口
        const res = await $http.get('/mental/emotion/stats/global')
        const data = res.data || {}
    ...
    async loadRiskDistribution() {
      ...
      try {
        // 请求风险分布接口
        const res = await $http.get('/mental/assessment/risk-distribution')
        const data = res.data || {}
    ...
    async loadRecentActivities() {
      ...
      try {
        ...
        const [emotionRes, assessmentRes] = await Promise.all([
          $http.get('/mental/emotion/recent'),
          $http.get('/mental/assessment/recent')
        ])
